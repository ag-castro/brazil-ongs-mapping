...
DEFAULT_QUALITY = getattr(
    settings,
    'DJANGORESIZED_DEFAULT_QUALITY',
    0
)
DEFAULT_KEEP_META = getattr(
    settings,
    'DJANGORESIZED_DEFAULT_KEEP_META',
    True
)
DEFAULT_FORCE_FORMAT = getattr(
    settings,
    'DJANGORESIZED_DEFAULT_FORCE_FORMAT',
    None
)
DEFAULT_FORMAT_EXTENSIONS = getattr(
    settings,
    'DJANGORESIZED_DEFAULT_FORMAT_EXTENSIONS',
    {}
)
DEFAULT_NORMALIZE_ROTATION = getattr(
    settings,
    'DJANGORESIZED_DEFAULT_NORMALIZE_ROTATION',
    True
)
...
class ImageResizerFieldFile(ImageField.attr_class):
    ...
    def save(self, name, content, save=True):
        ...
        if self.field.force_format and self.field.force_format.lower() in \
                ('jpeg', 'jpg') and img.mode != 'RGB':
        ...
        ImageFile.MAXBLOCK = max(
            ImageFile.MAXBLOCK, thumb.size[0] * thumb.size[1])
        new_content = BytesIO()
        img_format = img.format if self.field.force_format \
            is None else self.field.force_format
        thumb.save(new_content, format=img_format,
                   quality=self.field.quality, **img_info)
        new_content = ContentFile(new_content.getvalue())

        name = self.get_name(name, img_format)
        super(ImageResizerFieldFile, self).save(name, new_content, save)
...
class ImageResizerField(ImageField):

    attr_class = ImageResizerFieldFile

    def __init__(self, verbose_name=None, name=None, **kwargs):
        # migrate from 0.2.x
        depricated = ('max_width', 'max_height',
                      'use_thumbnail_aspect_ratio', 'background_color')
        for argname in depricated:
            if argname in kwargs:
                sys.stderr.write(
                    'Error: Keyword argument %s is deprecated'
                    ' for ImageResizerField' % argname)
                del kwargs[argname]
        ...
        super(ImageResizerField, self).__init__(verbose_name, name, **kwargs)

    def deconstruct(self):
        name, path, args, kwargs = super(ImageField, self).deconstruct()
        for custom_kwargs in [
            'crop', 'size', 'quality',
            'keep_meta', 'force_format'
        ]:
